...
-- Interests: each user's hobby/interest areas
...
-- Messages: chat messages within an interest
...
-- Courses: structured learning plans for an interest
...
-- Course Weeks: individual weeks within a course
...
-- Course Tasks: individual tasks within a week
...
-- Interests: users can CRUD their own interests
...
-- Messages: users can CRUD messages on their own interests
...
-- Courses: users can CRUD courses on their own interests
create policy "Users can view own courses"
  on courses for select
  using (
    exists (
      select 1 from interests
      where interests.id = courses.interest_id
        and interests.user_id = auth.uid()
    )
  );
...
-- Course Weeks: users can CRUD weeks on their own courses
create policy "Users can view own course weeks"
  on course_weeks for select
  using (
    exists (
      select 1 from courses
      join interests on interests.id = courses.interest_id
      where courses.id = course_weeks.course_id
        and interests.user_id = auth.uid()
    )
  );

create policy "Users can insert own course weeks"
...
-- Course Tasks: users can CRUD tasks on their own course weeks
create policy "Users can view own course tasks"
  ...
  using (
    exists (
      ...
      join interests on interests.id = courses.interest_id
      where course_weeks.id = course_tasks.week_id
        and interests.user_id = auth.uid()
    )
  );

create policy "Users can insert own course tasks"
  on course_tasks for insert
  with check (
    exists (
      select 1 from course_weeks
      join courses on courses.id = course_weeks.course_id
...
create policy "Users can update own course tasks"
  on course_tasks for update
  using (
    exists (
      select 1 from course_weeks
      join courses on courses.id = course_weeks.course_id
...
create policy "Users can delete own course tasks"
  on course_tasks for delete
  using (
    exists (
      select 1 from course_weeks
      join courses on courses.id = course_weeks.course_id
